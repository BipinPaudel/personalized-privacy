        ...
        for e0 in e0s:
            ...
            for minority_prob in probs:
                ...
                for exp_ins in experiment_types:
                    ...
                    eps_inf = 2 #1.5
                    eps_instantaneous = 1.1#1.2218197426699942 #0.6668319751834157 #0.8132616875182227 #This will give total 0.5
                    ...
                    if exp_ins == 'time':
                        ...
                        # epsilons = calculate_epsilons_for_interactions(eps_instantaneous, e0, eps_inf)
                        # epsilons = [[eps_inf, e0], [eps_inf, 0.25], [eps_inf, 0.05], [eps_inf, 0.05]]
                        rem_time = 1#248
                        # epsilons = np.array([[eps_inf, e0],[eps_inf, eps_instantaneous-e0]])
                        # epsilons = np.array([[eps_inf, e0], [eps_inf, 0.35]] + [[eps_inf, 0.1] for _ in range(rem_time)])
                        epsilons = np.array([[eps_inf, e0], [eps_inf, 0.1]] + [[eps_inf, 0.1/rem_time] for _ in range(rem_time)])
                        print(epsilons)
                    if exp_ins == 'single': 
                        ...
                        for time in range(len(epsilons)):
                            ...
                            if exp_ins == 'baseline':
                                ...
                                if estimated_histogram[0] < estimated_histogram[1]:
                                    l2.info('flipped')
                                dic_mse[seed][time] = calculate_error(real_freq, estimated_histogram, n)
                                ...
                                if estimated_histogram[0] < estimated_histogram[1]:
                                    l2.info('flipped')
                                dic_mse[seed][0] = calculate_error(real_freq, estimated_histogram, n)
                            else :
            ...
            l1.info(f'epsilon0: {e0}, epsilons: {epsilons}, interactions: {times}')
            l1.info(f'all_errors: {all_errors}')
            l1.info(f'ldp_errors: {ldp_errors}')
            l1.info(f'baseline_errors: {baseline_errors}')
            plot_errors(probs, all_errors,  ldp_errors, baseline_errors, e0, times, n, main_folder)