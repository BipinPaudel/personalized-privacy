import numpy as np
from protocols.grr import GRR_Client
...
def L_GRR_Client_Interaction(input_data, k, eps_perm, eps_1):
    # GRR parameters for round 1
        p1 = np.exp(eps_perm) / (np.exp(eps_perm) + k - 1)
        q1 = (1 - p1) / (k - 1)
        ...
        # Second round of sanitization with GRR using first_sanitization as input
        second_sanitization = GRR_Client(input_data, k, eps_sec_round)
        
        return [second_sanitization, p2, q2]
...
def GRR_Aggregator_IBU_interact(reports, k, latest_freq, epsilon_minority, epsilon_majority, interaction, ps, qs, 
                                sample=False, nb_iter=10000, tol=1e-12, err_func="max_abs"):
    """
    Estimator based on Iterative Bayesian Update[3,4].
    ...
    # Validations
    if sample:
        reports = np.random.choice(reports, size=len(reports)*0.4, replace=False)
    if len(reports) == 0:
        raise ValueError('List of reports is empty.')
    if not isinstance(k, int) or not isinstance(nb_iter, int) or k < 2:
        raise ValueError('k (>=2) and nb_iter need integer values.')
    ...
    n = len(latest_reports)

    majority_bin = np.array([])
    minority_bin = np.array([])

    for i, report in enumerate(latest_reports):
        all_reports_of_i_user = reports[i,: ]
        all_p = ps[i, :]
        all_q = qs[i, :]
        p_of_majority = 1
        p_of_minority = 1

        for j, report in enumerate(all_reports_of_i_user):
            if report == np.argmax(latest_freq):
                p_of_majority = p_of_majority * (all_p[0]*all_p[j+1] + all_q[0]*all_q[j+1])
            else:
                p_of_majority = p_of_majority * (all_p[0]*all_q[j+1] + all_q[0]*all_p[j+1])


        for report in all_reports_of_i_user:
            if report == np.argmin(latest_freq):
                p_of_minority = p_of_minority * (all_p[0]*all_p[i+1] + all_q[0]*all_q[i+1])
            else:
                p_of_minority = p_of_minority * (all_p[0]*all_q[i+1] + all_q[0]*all_p[i+1])
        
        scaled_p_of_majority = p_of_majority/(p_of_majority+p_of_minority)

        if np.random.binomial(1, scaled_p_of_majority) == 1:
            majority_bin.append(i)
        else:
            minority_bin.append(i)